...
#Zmena oproti swiftimp.sh, maska upravená pre potreby IBAN Directory Plus (Brandys, 10.08.2016)
...
MASKA="[E][X][C][L][U][S][I][O][N][L][I][S][T][_][V][0-9][_][D][A][I][L][Y]_[D][E][L][T][A][_][2][0][0-9][0-9][01][0-9][0-3][0-9].[Xx][Mm][Ll]"
...
#Zmena oproti swiftimp.h, meno doèasného súboru bude upravené pre potreby IBAN Directory Plus (Brandys, 10.08.2016)
#Pôvodná hodnota:TMP=${L_ADR}/tmp/tran_imp_tmp.$$
TMP=${L_ADR}/tmp/tran_iban_imp_exc_tmp.$$
#LOG subor do ktoreho sa loguje aktivita skriptu swift_imp.sh
#Zmena oproti swiftimp.h, meno log súboru bude upravené pre potreby IBAN Directory Plus (Brandys, 10.08.2016)
#Pôvodná hodnota:SUM_LOG=${L_ADR}/log/tran_imp.log
SUM_LOG=${L_ADR}/log/tran_iban_imp_exc.log

#pridane Branislav Brandys, premenne pre flag subor
FLAG_FILE=${L_ADR}/../work/uzav/bis_in/ibanexcl_sts.txt

LOG_SUB=${L_ADR}/${LOG_SUB}
#Zmena oproti swiftimp.h, cie¾ový adresár bude upravené pre potreby IBAN Directory Plus (Brandys, 10.08.2016)
#Pôvodná hodnota:L_ADR=${L_ADR}/fromswift
L_ADR=${L_ADR}/../work/uzav/bis_in/ibanexcl/davky
...
#####Check INPUT parameters <<
        ...
        12)
          ...
          ;;
#Zmena oproti swiftimp.h, pridan0 stavz pre potreby IBAN Directory Plus (Brandys, 10.08.2016)
		13)
...
#pridane Branislav Brandys 10.08.2016, kontrola flag suboru 
...
then
  ...
  #pridane pre ptreby importu iban plus Branislav Brandys (Brandys, 10.08.2016)
 if [ "${R_CODE}" == "0" ]
 then
	echo "1" > ${FLAG_FILE}
 fi
