...
#pridane pre potreby IBAN Plus inportu. Zadany log pre IBAN Plus
...
#pridane pre potreby IBAN
echo "`cas` PROPATH " >> $LOG_MAIN_IBAN
echo $PROPATH >> $LOG_MAIN_IBAN
...
if [ $POM -gt 1 ] 
  then 
    ...
    #pridane pre potreby IBAN
	echo "`cas` IMPORT INFO Script $SCRIPT_NAME je úž spustený" >> $LOG_MAIN_IBAN
    exit
fi
...
#pridane pre potreby IBAN
echo "`cas` IMPORT START" >> $LOG_MAIN_IBAN
...
if [ "$?" -ne "0" ]
  then
    ...
    echo "=" >> $LOG_MAIN
    #pridane pre potreby IBAN
	echo "`cas` IMPORT ERROR pri prenose suborov zo SWIFT ALLIANCE terminalu" >> $LOG_MAIN_IBAN
    echo "`cas` IMPORT STOP" >> $LOG_MAIN_IBAN
	echo "=" >> $LOG_MAIN_IBAN
    exit
fi
...
# pridane pre potreby IBAN Plus importu
# pokial vynikne pri importe swiftovych sprav chyba yastavi sa aj import IBAN plus

swift_iban_imp_str.sh `cat param_iban_imp_str.txt`
if [ "$?" -ne "0" ]
  then
    echo "`cas` IMPORT ERROR pri prenose IBAN STRUCTURE suborov zo SWIFT ALLIANCE terminalu" >> $LOG_MAIN_IBAN
    echo "`cas` IMPORT pokracuje na IBAN PLUS" >> $LOG_MAIN_IBAN
fi
swift_iban_imp.sh `cat param_iban_imp.txt`
if [ "$?" -ne "0" ]
  then
    echo "`cas` IMPORT ERROR pri prenose IBAN PLUS suborov zo SWIFT ALLIANCE terminalu" >> $LOG_MAIN_IBAN
    echo "`cas` IMPORT pokracuje na IBAN EXCLUSIONS" >> $LOG_MAIN_IBAN
fi
